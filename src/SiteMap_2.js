...
export default function SiteMap() {
  ...
  const headerStyle = { ...buttonStyle, background: '#f0f0f0', color: '#333', fontWeight: 'bold' };
  ...
  useEffect(() => {
    ...
    map.current.on('load', () => {
      ...
      map.current.on('mouseenter', 'sites-layer', () => map.current.getCanvas().style.cursor = 'pointer');
      map.current.on('mouseleave', 'sites-layer', () => map.current.getCanvas().style.cursor = '');
    });
  }, [data]);
  ...
  useEffect(() => {
    ...
    const expr = ['case', ['==', ['get', 'site'], selectedSite], '#FF4136', '#007cbf'];
      ...
      features: data.features
        .map(f => {
          const bs = f.properties.buildings.filter(b =>
            ['order','morphology','age','date'].every(attr => {
              const set = filters[attr];
              return !set.size || Array.from(set).some(v => b[attr].includes(v));
            })
          );
          return bs.length ? { ...f, properties: { ...f.properties, buildings: bs } } : null;
        })
        .filter(Boolean),
    };
  }, [data, filters]);
  ...
  useEffect(() => {
    if (map.current?.getSource('sites')) {
      map.current.getSource('sites').setData(filteredData);
    }
    if (selectedBuilding && !filteredData.features.some(f => f.properties.buildings.some(b => b.id === selectedBuilding))) {
      setSelectedBuilding(null);
    }
  }, [filteredData, selectedBuilding]);

  // Toggle filter or dropdown
  const toggleFilter = (attr, v) => setFilters(prev => {
    const next = { ...prev, [attr]: new Set(prev[attr]) };
    next[attr].has(v) ? next[attr].delete(v) : next[attr].add(v);
    return next;
  });
  const toggleDropdown = attr => setDropdownOpen(prev => ({ ...prev, [attr]: !prev[attr] }));
  const clearFilters = () => setFilters({ order: new Set(), morphology: new Set(), age: new Set(), date: new Set() });
  const exitSite = () => { setSelectedSite(null); setSelectedBuilding(null); };
  ...
  return (
    <div style={{ position: 'relative', height: '100vh', fontFamily: 'Arial, sans-serif' }}>
      ...
      <div style={{ position: 'absolute', top: 10, left: 10, background: '#fff', padding: '10px', borderRadius: '8px', boxShadow: '0 2px 6px rgba(0,0,0,0.15)', zIndex: 1 }}>
        <button onClick={clearFilters} style={{ ...buttonStyle, background: '#eee', color: '#333' }}>Clear Filters</button>
        ...
        <div style={{ marginTop: 8 }}>
          {Object.entries(filters).flatMap(([attr, set]) =>
            Array.from(set).map(v => (
              <button key={`${attr}-${v}`} onClick={() => toggleFilter(attr, v)} style={{ ...buttonStyle, background: '#007cbf', color: '#fff' }}>
                {v}
              </button>
            ))
          )}
      ...
      <div style={{ position: 'absolute', top: 0, right: 0, width: 320, height: '100%', background: '#fafafa', borderLeft: '1px solid #ddd', transform: siteFeat ? 'translateX(0)' : 'translateX(100%)', transition: 'transform 0.3s ease', padding: '16px', boxSizing: 'border-box', overflowY: 'auto', zIndex: 2 }}>
        <button onClick={exitSite} style={{ ...buttonStyle, position: 'absolute', bottom: 20, left: 20, background: '#ddd', color: '#333' }}>Exit Site</button>
            ...
            <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
              {siteFeat.properties.buildings.map(b => (
                <li key={b.id} style={{ marginBottom: '6px' }}>
                  <button onClick={() => setSelectedBuilding(b.id)} style={{ ...buttonStyle, background: '#eee', color: '#333' }}>
                    {b.id}
                  </button>
                </li>
              ))}
        ...
        {buildingObj && (
          <>
            ...
            <div style={{ marginBottom: '12px' }}>
              <strong>Doc:</strong>{' '}
              <a href={buildingObj.url} target="_blank" rel="noopener noreferrer" style={{ color: '#007cbf' }}>{buildingObj.doc_id}</a>
            </div>
            <button onClick={() => setSelectedBuilding(null)} style={{ ...buttonStyle, background: '#ddd', color: '#333' }}>Back</button>
          </>
        )}
      </div>
    </div>
